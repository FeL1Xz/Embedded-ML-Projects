...
class Server:
    ...
    def eval(self, data_test):
        ...
        numClasses, numClassVal = np.unique(data_test["y"], return_counts = True)
        ...
        win_accuracy = []
        win_f1 = []
        classcounter = np.zeros(len(numClasses))
        classsuc = np.zeros(len(numClasses))
        ...
        #np.savetxt(f, classcounter)
        print(classcounter)
        print(classsuc)
        testing = np.concatenate((classcounter, classsuc))
        print(testing)
        #testing = np.divide(classcounter, numClassVal)
        #print(testing)
        return np.mean(win_loss), np.mean(win_accuracy), np.mean(win_f1), classcounter, classsuc
