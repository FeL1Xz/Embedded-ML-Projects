...
class FL:
    ...
    def start(self):
        ...
        row = 0
        classcounterNew = np.zeros(18)
        classsucNew = np.zeros(18)
        ...
        for t in range(self.rounds):
            ...
            if t % self.eval_interval != 0:
                ...
                with torch.no_grad():
                    test_loss, test_accuracy, test_f1, classcounter, classsuc = server.eval(
                        server_test)
                result_table[row] = np.array(
                    (t+1, local_ae_loss, train_loss, train_accuracy, test_loss, test_accuracy, test_f1))
                row += 1

                for curClass in range(len(classcounter)):
                    classcounterNew[curClass] = np.add(classcounterNew[curClass], classcounter[curClass])
                    classsucNew[curClass] = np.add(classsucNew[curClass], classsuc[curClass])

                self.write_result(result_table)
                print(classcounterNew)
                print(classsucNew)

        combinedData = np.concatenate((classcounterNew, classsucNew))
        ...
        np.savetxt(os.path.join(results_path, "results.txt"),
                   combinedData, delimiter=",", fmt="%1.4e")
